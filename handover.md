...
- The Bresser laboratory PC runs Windows 10. Use the offline single-file build `dist\BresserCameraDashboard-Windows10.exe`; it excludes the Allied Vision/Vimba dependency, opens the browser directly, and saves captures in `%USERPROFILE%\Downloads`.
- The Bresser build registers `BresserSdkCameraDriver` before OpenCV. It loads `BresserCam.dll` or `toupcam.dll` from MikroCamLabII and lists proprietary devices as `Bresser MikroCam SDK: ...`. If its automatic DLL search fails, set `BRESSER_CAMERA_SDK_DLL` to the installed DLL's full path. OpenCV DirectShow and `USB webcam` entries are fallback paths only.
...
python app_bresser_windows10.py
```

## Windows EXE distribution

- Bresser Windows 10 offline build: `app_bresser_windows10.spec` produces the single file `dist\BresserCameraDashboard-Windows10.exe`.
